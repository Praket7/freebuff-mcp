...
export function normalizeProgressEvent(value: unknown, eventName?: string): Omit<ThreadProgressEvent, 'sequence'> | null {
  ...
  const out: Omit<ThreadProgressEvent, 'sequence'> = { threadId, timestamp, kind, phase, state:text(redact(input.state)) ?? text(redact(input.turnState)) ?? text(redact(input.status)), tool:text(redact(tool)), command:text(redact(command)), text: rawType.includes('reasoning') ? undefined : text(redact(input.text)) ?? text(redact(input.content)) ?? text(redact(input.message)), files: files?.map(file => String(redact(file))), error:text(redact(input.error)) };
...
export class ProgressStore {
  ...
  async wait(threadId: string, afterSequence = 0, timeoutMs = 30_000, limit = 50): Promise<ThreadProgressSnapshot> { return await new Promise(resolve => { const set = this.waiters.get(threadId) ?? new Set<() => void>(); let settled = false; const cleanup = () => { clearTimeout(timer); set.delete(wake); if (!set.size) this.waiters.delete(threadId); }; const wake = () => { if (settled) return; const current = this.read(threadId, afterSequence, limit); if (!current.events.length) return; settled = true; cleanup(); resolve(current); }; set.add(wake); this.waiters.set(threadId, set); const timer = setTimeout(() => { if (settled) return; settled = true; cleanup(); resolve(this.read(threadId, afterSequence, limit)); }, Math.min(Math.max(timeoutMs, 0), 30_000)); wake(); }); }
}

export class DesktopEventClient {
  private controller?: AbortController; private disposed = false; private running = false;
  constructor(private readonly base: () => URL, private readonly launchId: () => string | undefined, private readonly store: ProgressStore, private readonly refresh?: () => Promise<void>) {}
  ...
  private async loop(): Promise<void> { let delay = 250; let refreshed = false; while (!this.disposed) { this.controller = new AbortController(); try { const response = await fetch(new URL('/api/events', this.base()), { headers: { accept:'text/event-stream', ...(this.launchId() ? {'x-freebuff-launch-id':this.launchId()!} : {}) }, signal:this.controller.signal }); if ([401,403,404].includes(response.status) && !refreshed) { refreshed = true; await this.refresh?.(); continue; } if (!response.ok || !response.body) throw new Error(`SSE HTTP ${response.status}`); refreshed = false; this.store.setConnected(true); delay = 250; await this.consume(response.body); } catch { this.store.setConnected(false); } finally { this.controller = undefined; } if (!this.disposed) { await new Promise<void>(resolve => { const timer = setTimeout(resolve, delay); timer.unref?.(); }); delay = Math.min(delay * 2, 10_000); } } this.running = false; }
