...
export class DesktopOrchestratorRuntime implements Runtime {
  ...
  async capabilities(): Promise<Capabilities> { if (this.caps) return this.caps; const candidate = this.explicitBase ? { url:this.explicitBase, launchId:process.env.FREEBUFF_LAUNCH_ID } : await discoverDesktopCandidate(); if (!candidate) { this.caps = { product:'unknown', signedIn:'unknown', orchestrator:false, readOnly:true, endpoints:[], status:'not_found', liveProgress:'unavailable', selectedRuntime:'none', notes:['No Freebuff Desktop or CLI was selected. Run doctor for setup details.'] }; return this.caps; } this.base = new URL(candidate.url); this.launchId = candidate.launchId; try { const projects = await this.request<unknown>('GET','/api/projects'); const record=asRecord(projects); if (!record || !Array.isArray(record.projects)) throw new Error('invalid /api/projects response'); let writable = false; if (this.launchId) { try { const health = await this.request<unknown>('GET','/healthz'); writable = Boolean(asRecord(health)?.ok === true); } catch { writable = false; } } this.events.start(); this.caps = { product:'desktop', signedIn:'unknown', orchestrator:true, readOnly:!writable, status:writable?'desktop_writable':'desktop_read_only', liveProgress:'connected', selectedRuntime:'desktop', endpoints:['/api/projects','/api/thread/:id','/api/thread/:id/attachment','/api/events',...(writable ? ['/api/thread/:id/message','/api/thread/:id/stop','/api/thread/:id/resume','/api/thread/:id/agent','/api/thread/:id/effort'] : [])], notes:[`Desktop connected at ${candidate.url}${candidate.pid ? ` (PID ${candidate.pid})` : ''}.`, writable ? 'Desktop connected with verified writes through health checks.' : 'Desktop connected read only because launch authorization is missing or stale.', 'Live progress is connected and may become stale if Desktop stops sending events.'] }; } catch (error) { this.caps = { product:'unknown', signedIn:'unknown', orchestrator:false, readOnly:true, endpoints:[], status:'not_found', liveProgress:'unavailable', selectedRuntime:'none', notes:[`Desktop discovery reached ${candidate.url}, but its response was invalid or unavailable. ${error instanceof Error ? error.message : 'Unknown error'}`] }; } return this.caps; }
